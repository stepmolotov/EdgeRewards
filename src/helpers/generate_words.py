...
def generate_words(n_words: int = 10, min_length: int = 4, file_path: Optional[str] = None) -> List[str]:
    if file_path:
        return generate_words_from_file(file_path=file_path, n_words=n_words, min_length=min_length)
    else:
...
def generate_words_from_file(file_path: str, n_words: int = 10, min_length: int = 4) -> List[str]:
    """
